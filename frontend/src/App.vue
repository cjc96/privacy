      ...
      <b-row>
        <b-col cols='3' class='right-line'>
          <b-row v-for="(item,index) in types" :key="index" class='custom-input'>
            ...
            <b-col cols="9"><b-form-input :id="`input-${item}`" v-model="input_info[item]" :value="input_info[item]"></b-form-input></b-col>
          </b-row>
          <b-row class='custom-input'>
            <b-col cols="3"><label :for="`input-Epsilon`">Epsilon:</label></b-col>
            <b-col cols="9"><b-form-input :id="`input-Epsilon`" v-model="input_info['Epsilon']" :value="input_info['Epsilon']" :type="'range'" min=0.05 max=1 step=0.05></b-form-input></b-col>
          </b-row>
          <b-row class='custom-input'>
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      let url = host + `get_ans?location=${currentName}&xmin=${this.input_info.xmin}&ymin=${this.input_info.ymin}&xmax=${this.input_info.xmax}&ymax=${this.input_info.ymax}&epsilon=${this.input_info.Epsilon}`;
      fetch(url)
      .then(res => res.json())
      .then(res => {
